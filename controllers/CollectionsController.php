...
use app\models\CollectionsSearch;
...
class CollectionsController extends Controller
{
    ...
    public function behaviors()
    {
        return [
            'access' => [
                ...
                'rules' => [
                    [
                        'actions' => ['index','amo-collection','create','view','update'],
                        'allow' => true,
                        'roles' => ['@'],
                    ],
                    [
    ...
    public function actionAmoCollection()
    {
        $amo = new Amo(\Yii::$app->params);
        $fields = $amo->getAccounts();
        foreach ($fields['account']['custom_fields']['leads'] as $field){
            if($field['id'] == 1286504){
                foreach ($field['enums'] as $key => $value){
                    $model = CollectionsSearch::findOne(['label' => $value]);
                    if(!$model){
                        $model = new Collections();
                        $model->setAttribute('label',$value);
                        $model->save();
                       // var_dump($model);
                    }
                }
                //  var_dump($field['enums']);
