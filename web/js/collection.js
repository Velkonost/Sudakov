    ...
    if ($(".diagram-container").length > 0) {
        ...
        $(".mainPage-rightBlock-body-Checkbox").on("change", function () {
            ...
            if ($(".ch-" + id).prop("checked")) {
                $(".id-" + id).addClass("colorcol" + id + " colored");
            } else {
                $(".id-" + id).removeClass("colorcol" + id + " colored");
            }
        });
        $(".cb-collection.ch-1").prop("checked", true);
        $(".cb-collection.ch-1").trigger("change");
        ...
        function createPlot3(startDate, finishDate) {
            ...
            plot3 = $.jqplot("main_chart", [[[["2000", 2], ["2017", 40]]]], {
                //height: 400,
                width: "100%",
                title: "",
                series: [{label: 'Лиды'}],
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.code == 0) {
                        ...
                        //}
                        collectionsColor = data.colorColl;
                        var chartcount = [['Коллекция','Штук',{type: 'string', role: 'tooltip', 'p': {'html': true}}]];
                        var chartsumm = [['Коллекция','Деньги',{type: 'string', role: 'tooltip', 'p': {'html': true}}]];
                        var count_temp;
                        var count_temp1;
                        var summ_temp;
                        var summ_temp1;
                        //  chartar.push(['Task', 'Hours per Day']);
                        for (var i in data.chartresult) {


                            if(~i.indexOf("count")){
                                count_temp = i;
                                count_temp1 = data.chartresult[i];
                                continue;
                            }

                            if(~i.indexOf("summ")){
                                summ_temp = i;
                                summ_temp1 = data.chartresult[i];
                                continue;
                            }

                            chartcount.push([count_temp,count_temp1,data.chartresult[i]]);
                            chartsumm.push([summ_temp,summ_temp1,data.chartresult[i]]);
                        ...
                        for (var i in facts) {
                            ...
                            $(".fv_collection_" + i).html(facts[i]);
                        }

                        gs = 0;
                        for (var i in data.result_average) {
                            gs++;
                            $(".av_collection_" + gs).html(data.result_average[i]);
                            gs++;
                        }
                        ...
                        for (var i in data.changes) {
                            ...
                            var $stastusBlock = $(".fc_collection_" + gs);
                            ...
                            $(".fc_collection_" + gs).html(data.changes[i]);
                            gs++;
                        }
                        ...
                        for (var i in data.cities_plan) {
                            ...
                            $(".fpv_collection_" + i).html(data.cities_plan[i].value);
                            $stastusPlanBlock.html(data.cities_plan[i].change);
                        }
                        // Процент
                        for (var i in data.ratio) {
                            $(".ratio_collection_" + i).html(data.ratio[i] + " %");
                        }
                    }
                    ...
                    function drawChart1() {
                        //alert(chartcount);
                        var datacount = google.visualization.arrayToDataTable(chartcount);
                        var datasumm = google.visualization.arrayToDataTable(chartsumm);
                        var optionscount = {
                            tooltip: {isHtml: true,ignoreBounds:false,showColorCode:true,trigger:'selection'},
                            title: 'Количество в коллекциях',
                            // is3D: true,
                            legend:{position: 'none',alignment: 'center'},
                            pieSliceText:'value',
                            enableInteractivity:true,
                            colors: collectionsColor
                        };
                        var optionssumm = {
                            tooltip: {isHtml: true,ignoreBounds:false,showColorCode:true,trigger:'selection'},
                            title: 'Сумма в коллекциях',
                            // is3D: true,
                            legend:{position: 'none',alignment: 'center'},
                            pieSliceText:'value',
                            enableInteractivity:true,
                        ...
                        var chart2 = new google.visualization.PieChart(document.getElementById('piechart_3d_count'));
                        var chart3 = new google.visualization.PieChart(document.getElementById('piechart_3d_summ'));
                        google.visualization.events.addListener(chart2, 'click', selectHandler2);
                        google.visualization.events.addListener(chart3, 'click', selectHandler3);

                        chart2.draw(datacount, optionscount);
                        chart3.draw(datasumm, optionssumm);
