...
define(['jquery'], function($){
    ...
    return function () {
 		var self = this,
			...
			managerId = 0;
		const domainName = 'https://sergeysudakov.ru';
		...
		self.setLeadStatus = function(leadId, managerId, status, domainName, self, callback) {
            ...
            $.ajax({ url: domainName + '/erp-amo/set-allocation', dataType: "jsonp", data: data }).done(function (json) {
                if(json.status == 200) {
		...
		self.watchRequest = function() {
            ...
            $.ajax({
            	url: domainName + '/erp-amo/allocation-request',
				dataType: "jsonp",
